...
import com.amazonaws.services.ec2.model.{DescribeSpotPriceHistoryRequest, RequestSpotFleetRequest}
import com.typesafe.scalalogging.slf4j.Logger
import in.ashwanthkumar.matsya.spotfleet.SpotFleetStateStore
...
/*
	def syncSpotFleetClusterSettings(): Unit = {
		config.spotFleetClusters
			...
			.foreach { c =>
				val clusterConf = c.config
				logger.info(s"New spot fleet cluster ${clusterConf.getClientToken} found")
				val request = new RequestSpotFleetRequest().withSpotFleetRequestConfig(clusterConf)
			}
...
*/
	...
	def checkClusters(): Unit = {
		checkASGClusters()
		checkSpotFleetClusters()
	}

	def checkASGClusters(): Unit = {
		config.asgClusters.foreach(clusterConfig => {
			val previousState = stateStore.get(clusterConfig.name).asInstanceOf[State]
	...
	def checkSpotFleetClusters(): Unit = {
		config.spotFleetClusters.foreach { clusterConfig =>
			val request = new RequestSpotFleetRequest().withSpotFleetRequestConfig(clusterConfig.config)
